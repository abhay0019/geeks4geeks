...
choice_to_category = {1: 'c', 2: 'c-plus-plus', 3: 'java',
                      ...
                      5: 'data-structures',6:'python',7:'project',8:'interview-corner',9:'gfact',10:'program-output',11:'design'}
...
def scrape_category(categoryUrl):
    ...
    try:
        ...
        if categoryUrl=='project':
          ...
          xtra='about/'
        elif categoryUrl=='gfact' or categoryUrl=='program-output' or categoryUrl=='design':
          xtra='category/'
    ...
    if categoryUrl=='project':
      ...
      links=[]
      t1links=[a.attrs.get('href') for a in soup.select('article ul li a')]
      for link in t1links:
      ...
      for i in range(2,num+1):
        ...
        soup1 = BeautifulSoup(requests.get(link+"/page/"+str(i)).text)
        links.extend([a.attrs.get('href') for a in soup1.select('h2.entry-title a')])
    elif categoryUrl=='program-output' or categoryUrl=='design':
      links=[]
      print "Output"
      links.extend([a.attrs.get('href') for a in soup.select('h2.entry-title a')])
      pages=soup.select('span.pages')
